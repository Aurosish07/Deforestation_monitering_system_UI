...
export const api = {
    ...
    getJobStatus: async (job_id: string): Promise<JobStatus> => {
        ...
        return {
            job_id: data.task_id,
            status: statusMapped,
            progress: statusMapped === 'done' ? 100 : statusMapped === 'error' ? 0 : 50,
            error: data.error,
            stage: data.stage,
            summary: data.n_incidents ? {
                total_area_ha: 0,
                incident_count: data.n_incidents,
            } : undefined,
        };
    },
    ...
    getIncidents: async (job_id: string): Promise<Incident[]> => {
        ...
        return rawData.features.map((feature: any, index: number) => {
            const severityLevel = feature.properties.area_ha > 30 ? 'critical'
                : feature.properties.area_ha > 10 ? 'high'
                    : 'medium';
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                resolve({
                    ...
                    summary_text: "Automated API analysis executed entirely via Bounding Box coordinate targeting.",
